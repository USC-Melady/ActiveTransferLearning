function [ stats ] = bottom_up_update(tree, stats, leaf, to_node, labels)

n_labels = size(stats.labelCounts,2);
yl = labels(leaf);
wl = tree.leafCounts(leaf);

path = get_path_ascending(tree, leaf, to_node);
for i=1:length(path)
    v = path(i);
    stats.labelCounts(v,yl) = stats.labelCounts(v,yl) + wl;
    ...
    % QUESTION: should admissibility persist? Paper says it should, but
    % DH's implementation does not.
    adm = all(LB > UB, 1); % no, a la DH's code
%         adm = stats.admissible(v,:) | all(LB > UB, 1); % yes, a la paper
...
%         sc = sc - est; % use estimate, a la paper
    ...
    if v > tree.n_leaves && stats.bestLabel(v) ~= 0
        ch = tree.children(v,:);
        ...
        if sc_split < sc
            ...
            stats.score(v) = sc;
            % QUESTION: should split-ability persist? Paper not clear
            % but DH's implementation does.
%                 stats.canSplitNode(v) = 0; % no
        end
    else
        stats.score(v) = sc;
